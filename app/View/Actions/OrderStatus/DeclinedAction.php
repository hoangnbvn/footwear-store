...
class DeclinedAction extends Action
{
    /**
    ...
    public function handle($model, View $view)
    {
        ...
        if ($model->status == config('app.bill_status.pending')) {
            $model->status = config('app.bill_status.declined');
            $model->save();
    ...
    {
        return $model->status == config('app.bill_status.pending');
    }
}
