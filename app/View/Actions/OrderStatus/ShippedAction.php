...
class ShippedAction extends Action
{
    /**
    ...
    public function handle($model, View $view)
    {
        ...
        if ($model->status == config('app.bill_status.shipping')) {
            $model->status = config('app.bill_status.shipped');
            $model->save();
    ...
    {
        return $model->status == config('app.bill_status.shipping');
    }
}
