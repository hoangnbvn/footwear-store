        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('users.store') }}">
                ...
                @if (session('status') === 'user-created')
                        <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)" class="text-sm text-gray-600 dark:text-gray-400">{{ __('Admin user is created.') }}</p>
                        @endif
                    <x-button class="ml-4">
                        {{ __('Create') }}
                    </x-button>
